...
-- 程序
--
CREATE DEFINER=`root`@`localhost` PROCEDURE `create_a_like_item` (IN `in_OrderAmount` INT, IN `in_Account` VARCHAR(16) CHARSET big5, IN `in_TotalFee` DECIMAL(10,5), IN `in_TotalAmount` DECIMAL(10,5), IN `in_UserID` VARCHAR(10) CHARSET big5, IN `in_Product_No` BIGINT)   INSERT INTO `likelist` (`OrderAmount`, `Account`, `TotalFee`, `TotalAmount`, `UserId`, `Product_No`) VALUES (in_OrderAmount, in_Account, in_TotalFee, in_TotalAmount, in_UserID, in_Product_No)$$
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `update_a_like_item` (IN `in_SN` BIGINT, IN `in_OrderAmount` INT, IN `in_Account` VARCHAR(16), IN `in_TotalFee` DECIMAL(10,5), IN `in_TotalAmount` DECIMAL(10,5), IN `in_UserID` VARCHAR(10), IN `in_Product_No` BIGINT)   UPDATE `likelist` SET `OrderAmount`=in_OrderAmount, `Account`=in_Account, `TotalFee`=in_TotalFee, `TotalAmount`=in_TotalAmount, `UserId`=in_UserID, `Product_No`=in_Product_No WHERE `SN` = in_SN$$
...
CREATE TABLE `likelist` (
  ...
  `TotalFee` decimal(10,5) NOT NULL,
