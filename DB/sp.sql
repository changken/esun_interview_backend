...
DELIMITER $$
CREATE DEFINER=`root`@`localhost` PROCEDURE `create_a_like_item`(IN `in_OrderAmount` INT, IN `in_Account` VARCHAR(16) CHARSET big5, IN `in_TotalFee` DECIMAL(10,5), IN `in_TotalAmount` DECIMAL(10,5), IN `in_UserID` VARCHAR(10) CHARSET big5, IN `in_Product_No` BIGINT)
...
DELIMITER $$
CREATE DEFINER=`root`@`localhost` PROCEDURE `update_a_like_item`(IN `in_SN` BIGINT, IN `in_OrderAmount` INT, IN `in_Account` VARCHAR(16), IN `in_TotalFee` DECIMAL(10,5), IN `in_TotalAmount` DECIMAL(10,5), IN `in_UserID` VARCHAR(10), IN `in_Product_No` BIGINT)
